...
def _run_czsc_scan():
    ...
    # Saturday morning catch-up: if Fri scan wasn't done, try 08:00-12:00
    if now.weekday() == 5:  # Saturday
        ...
        if os.path.exists(fri_marker) or os.path.exists(today_marker):
            return  # Friday already done
        # Use Friday's date for logging
        scan_date = fri
        logger.info("周六补跑: 检查周五(%s)数据扫描", fri)
    ...
    # Check if already done today (or for the target scan date)
    done_marker = os.path.join(_RUN_MARKER_DIR, f"czsc_scan_done_{scan_date}")
    if os.path.exists(done_marker):
        return

    # Throttle: wait at least 15 minutes between attempts
    ...
    try:
        with open(status_file, "r") as f:
            status = json.load(f)
            if status.get("scan_date") == scan_date:
                retry_count = status.get("retry_count", 0)
                last_attempt_time = status.get("last_attempt", "")
    except Exception:
        pass

    if last_attempt_time:
        try:
            last_dt = datetime.strptime(last_attempt_time, "%Y-%m-%d %H:%M:%S")
    ...
    logger.info("czsc扫描开始 (目标:%s, 第%d次尝试)...", scan_date, attempt_num)
    _log_event("scan_attempt", "czsc_scan", "info",
               f"开始扫描(目标:{scan_date}, 第{attempt_num}次尝试)")
    ...
    try:
        ...
        if not _scan_ok:
            ...
            if reason == "data_not_ready":
                _log_event("scan_retry", "czsc_scan", "warn",
                           f"数据未就绪，第{retry_count}次重试(15分钟后再试)")
            else:
                _log_event("scan_error", "czsc_scan", "error",
        ...
        try:
            ...
            pass
        _log_event("scan_timeout", "czsc_scan", "error",
                   f"扫描超时(第{retry_count}次, 15分钟后重试)")
    except Exception as e:
...
def _check_pipeline_health():
    """Check if pipeline daemon is alive; restart if dead."""
    global _PIPELINE_FAIL_COUNT
    now = datetime.now()
    # Check during broader window: 9:00 - 22:00 (covers collection + scan)
    if now.hour < 9 or now.hour >= 22:
        return
    # Weekend skip (daemon handles its own schedule)
    if now.weekday() >= 5:
        return
    ...
    try:
        ...
        for line in ps_result.stdout.split("\n"):
            if "pipeline.py" in line and "grep" not in line:
                if "--mode daemon" in line:
                    daemon_alive = True
                if "--mode daily" in line:
                    daily_running = True
    except Exception:
        pass

    # Daemon is the critical process; if it's dead, try to restart
    if daemon_alive:
        _PIPELINE_FAIL_COUNT = 0
        return
    ...
    if _PIPELINE_FAIL_COUNT > _PIPELINE_MAX_FAILS:
        logger.warning("Pipeline daemon 连续 %d 次未检测到，停止自动恢复", _PIPELINE_FAIL_COUNT)
        _log_event("pipeline_max_retries", "pipeline", "critical",
                   f"Pipeline daemon 连续 {_PIPELINE_FAIL_COUNT} 次未检测到，需人工介入")
        return

    logger.warning("Pipeline daemon 不存在，尝试自动恢复 (第%d次)", _PIPELINE_FAIL_COUNT)
    _log_event("pipeline_failure", "pipeline", "error",
               f"Pipeline daemon 进程不存在，第 {_PIPELINE_FAIL_COUNT} 次自动恢复尝试")
    ...
    try:
        ...
        logger.info("Pipeline daemon 自动恢复启动成功")
        _log_event("pipeline_recovery", "pipeline", "info",
                   f"Pipeline daemon 自动恢复成功 (第 {_PIPELINE_FAIL_COUNT} 次)")
    except Exception as e:
        logger.error("Pipeline daemon 自动恢复失败: %s", e)
        _log_event("pipeline_recovery_failed", "pipeline", "error",
                   f"Pipeline 自动恢复失败: {str(e)[:200]}")


# ============================================================
# NEW: Data collection monitor - track progress and log events
# ============================================================
_DATA_STATUS_FILE = "/var/log/d8q/data_status.json"
_data_status_cache = {"_date": None}

def _monitor_data_collection():
    """Monitor daily data collection progress; log key milestones to system_events."""
    now = datetime.now()
    today = now.strftime("%Y-%m-%d")

    # Only monitor on weekdays during collection window (15:00 - 22:00)
    if now.weekday() >= 5:
        return
    ...
    try:
        ...
        db_cfg = {
            ...
            "user": "d8q_remote", "password": "d8q_remote_pass_2026",
        ...
        latest_count = rows[0]["cnt"]
        ref_count = rows[1]["cnt"] if len(rows) > 1 else latest_count
        ref_date = str(rows[1]["date"]) if len(rows) > 1 else ""

        # Load previous status
        prev = {}
        try:
            with open(_DATA_STATUS_FILE, "r") as f:
                prev = json.load(f)
        except Exception:
            pass

        curr = {"date": latest_date, "count": latest_count,
                "ref_count": ref_count, "ref_date": ref_date,
                "checked_at": now.strftime("%Y-%m-%d %H:%M:%S")}

        # Detect state transitions
        prev_state = prev.get("state", "")
        is_latest_today = (latest_date == today)
        is_ready = ref_count > 0 and latest_count >= ref_count * 0.95

        # State: collecting -> ready
        if is_latest_today and is_ready and prev_state != "ready":
            curr["state"] = "ready"
            _log_event("data_ready", "data_collection", "info",
                       f"数据采集完成: {latest_date} 共{latest_count}只 "
                       f"(参考{ref_date}={ref_count}只, 达{latest_count/ref_count*100:.1f}%)",
                       {"date": latest_date, "count": latest_count, "ref_count": ref_count})
            logger.info("数据就绪: %s %d只 (%.1f%%)", latest_date, latest_count,
                        latest_count/ref_count*100 if ref_count else 0)
        elif is_latest_today and not is_ready and prev_state not in ("collecting", "ready"):
            curr["state"] = "collecting"
            _log_event("data_collecting", "data_collection", "info",
                       f"数据采集中: {latest_date} 已入库{latest_count}只 "
                       f"(目标~{ref_count}只, {latest_count/ref_count*100:.1f}%)",
                       {"date": latest_date, "count": latest_count, "ref_count": ref_count})
            logger.info("数据采集中: %s %d/%d (%.1f%%)", latest_date, latest_count,
                        ref_count, latest_count/ref_count*100 if ref_count else 0)
        elif is_latest_today and is_ready:
            curr["state"] = "ready"
        elif is_latest_today:
            curr["state"] = "collecting"
        else:
            curr["state"] = "waiting"

        # Check for stale data (latest date is not today and it's past 17:00)
        if not is_latest_today and now.hour >= 17:
            curr["state"] = "stale"
            if prev_state != "stale":
                _log_event("data_stale", "data_collection", "warn",
                           f"数据可能延迟: 最新日期={latest_date}, 当前={today}, "
                           f"已过17:00仍未入库",
                           {"latest_date": latest_date, "today": today})

        # Save status
        with open(_DATA_STATUS_FILE, "w") as f:
            json.dump(curr, f, ensure_ascii=False)

        _data_status_cache["_date"] = today
        _data_status_cache.update(curr)

    except Exception as e:
        logger.error("数据采集监控异常: %s", e)
